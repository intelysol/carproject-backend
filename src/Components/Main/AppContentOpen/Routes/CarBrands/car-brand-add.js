...
const CarBrandForm = () => {
  ...
  const apiCall = async (carBrandName, status) => {
    let apiUrl = `${host[0].hostUrl}/api/post/data/carBrands`;

    try {
      let response = await axios({
        method: "POST",
        url: apiUrl,
        data: { carBrandName, status },
      });
      console.log(response);
      if (response.status == 200) {
        uploadImage();
  ...
  const addNewCarBrand = async () => {
    if (brandName.trim() == "" || file == null) {
      ...
      apiCall(brandName.trim(), statusValue);
    }
  };
            ...
            <div className="card-body">
              ...
              >
                ...
                <div className="col-12">
                  ...
                  <div className="btn btn-primary" onClick={addNewCarBrand}>
                    Add New Car Brand
                  </div>
                </div>
              </form>
