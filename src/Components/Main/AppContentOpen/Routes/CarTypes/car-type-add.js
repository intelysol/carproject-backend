...
const CarTypeForm = () => {
  ...
  const apiCall = async (carTypeName, typeAbbrevation, status) => {
    ...
    try {
      let response = await axios({
        method: "POST",
        url: apiUrl,
        data: { carTypeName, typeAbbrevation, status },
      });
  ...
  const addNewCarType = async () => {
    if (carTypeName.trim() == "" || abbrevation.trim() == "") {
      ...
      apiCall(carTypeName.trim(), abbrevation.trim(), statusValue);
    }
  };
